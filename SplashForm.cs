...
<body>
<video id="liaVideo" autoplay playsinline preload="auto">
  <source src="https://lia-splash.local/lia_abertura_pro.mp4" type="video/mp4">
</video>
<script>
const v=document.getElementById('liaVideo');
v.addEventListener('ended',()=>chrome.webview.postMessage('lia-video-ended'));
v.addEventListener('error',()=>chrome.webview.postMessage('lia-video-ended'));
v.play().catch(()=>{});
</script>
</body>
</html>
