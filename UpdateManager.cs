...
{
    public const string CurrentVersion = "10.136";
    ...
    public static async Task CheckForUpdatesAsync(IWin32Window? owner, bool silent)
    {
        try
        {
            ...
            if (manifest == null) { if (!silent) MessageBox.Show(owner, "O atualizador está instalado e funcionando.\n\nNenhuma publicação de atualização foi encontrada agora.", "Atualizações do LEAL INFO PDV", MessageBoxButtons.OK, MessageBoxIcon.Information); return; }
        ...
        catch (Exception ex) { if (!silent) MessageBox.Show(owner, "Não foi possível verificar atualizações agora.\n\n" + ex.Message, "Atualizações do LEAL INFO PDV", MessageBoxButtons.OK, MessageBoxIcon.Warning); }
    }

    public static void ShowUpdateCenter(IWin32Window owner)
    {
        using var f = new Form { Text = "Atualizações do LEAL INFO PDV", StartPosition = FormStartPosition.CenterParent, Width = 640, Height = 410, FormBorderStyle = FormBorderStyle.FixedDialog, MaximizeBox = false, MinimizeBox = false, BackColor = Color.FromArgb(7,31,52), Font = new Font("Segoe UI",10) };
        var title = new Label { Text="CENTRAL DE ATUALIZAÇÕES", ForeColor=Color.White, Font=new Font("Segoe UI",20,FontStyle.Bold), TextAlign=ContentAlignment.MiddleCenter, Dock=DockStyle.Top, Height=74 }; f.Controls.Add(title);
        var version = new Label { Text=$"VERSÃO INSTALADA  •  V{CurrentVersion}", ForeColor=Color.FromArgb(74,215,255), Font=new Font("Segoe UI",13,FontStyle.Bold), TextAlign=ContentAlignment.MiddleCenter, Dock=DockStyle.Top, Height=44 }; f.Controls.Add(version);
        var info = new Label { Text="O LEAL INFO PDV pode procurar novas versões e instalar a atualização sem reinstalar o sistema.\nSeus cadastros e banco de dados permanecem preservados.", ForeColor=Color.Gainsboro, TextAlign=ContentAlignment.MiddleCenter, Dock=DockStyle.Top, Height=78, Padding=new Padding(35,8,35,0) }; f.Controls.Add(info);
        var status = new Label { Text="● ATUALIZAÇÃO AUTOMÁTICA ATIVA", ForeColor=Color.FromArgb(105,240,170), Font=new Font("Segoe UI",11,FontStyle.Bold), TextAlign=ContentAlignment.MiddleCenter, Dock=DockStyle.Top, Height=42 }; f.Controls.Add(status);
        var check = new Button { Text="VERIFICAR ATUALIZAÇÕES AGORA", Width=310, Height=48, Left=155, Top=258, BackColor=Color.FromArgb(0,133,194), ForeColor=Color.White, FlatStyle=FlatStyle.Flat, Font=new Font("Segoe UI",10,FontStyle.Bold), Cursor=Cursors.Hand }; check.FlatAppearance.BorderSize=0;
        check.Click += async (_,_) => { check.Enabled=false; check.Text="VERIFICANDO..."; await CheckForUpdatesAsync(f,false); if(!f.IsDisposed){check.Text="VERIFICAR ATUALIZAÇÕES AGORA";check.Enabled=true;} }; f.Controls.Add(check);
        var close = new Button { Text="FECHAR", Width=130, Height=38, Left=245, Top=320, BackColor=Color.FromArgb(32,49,65), ForeColor=Color.White, FlatStyle=FlatStyle.Flat }; close.FlatAppearance.BorderColor=Color.FromArgb(80,120,145); close.Click += (_,_)=>f.Close(); f.Controls.Add(close); f.ShowDialog(owner);
    }
    ...
    private static async Task<UpdateManifest?> LoadManifestAsync()
    {
        ...
        using var http=new HttpClient{Timeout=TimeSpan.FromSeconds(7)}; http.DefaultRequestHeaders.UserAgent.ParseAdd("LEAL-INFO-PDV-Updater/10.136"); using var response=await http.GetAsync(feed); if(!response.IsSuccessStatusCode)return null; var json=await response.Content.ReadAsStringAsync(); return JsonSerializer.Deserialize<UpdateManifest>(json,JsonOptions());
    }
    ...
    private static async Task DownloadAndApplyAsync(UpdateManifest manifest,IWin32Window? owner)
    {
        ...
        var ps=Path.Combine(work,"apply-update.ps1");var log=Path.Combine(Database.AppFolder,"Updates","apply-update.log");
        var script=$$"""
        ...
        try{Process.Start(new ProcessStartInfo{FileName="powershell.exe",Arguments=$"-NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -File \"{ps}\"",UseShellExecute=true,Verb="runas",WorkingDirectory=work});}
        catch(System.ComponentModel.Win32Exception ex) when(ex.NativeErrorCode==1223){throw new InvalidOperationException("A atualização precisa da autorização do Windows. Clique em SIM na janela de Controle de Conta de Usuário para concluir.");}
        MessageBox.Show(owner,"A atualização foi preparada.\n\nO PDV será fechado, os arquivos serão atualizados e o sistema abrirá novamente automaticamente.\n\nSe o Windows pedir autorização, clique em SIM.","LEAL INFO PDV",MessageBoxButtons.OK,MessageBoxIcon.Information);Application.Exit();
    }
