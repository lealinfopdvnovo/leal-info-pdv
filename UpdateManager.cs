...
{
 public const string CurrentVersion="10.142";
...
try {
 ...
 Get-ChildItem -Path $stage -File -Recurse | ForEach-Object {
  ...
  $ok=$false
  for($i=0;$i -lt 10 -and -not $ok;$i++){ try { Copy-Item -Force $_.FullName $dest; $ok=$true } catch { Start-Sleep -Milliseconds 500 } }
...
""";
  ...
  Atualizar(100,"INSTALANDO E REINICIANDO...");await Task.Delay(350);progresso.Close();Application.Exit();
 }
