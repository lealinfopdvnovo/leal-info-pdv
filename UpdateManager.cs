...
{
 public const string CurrentVersion="10.164";
 ...
 private static async Task<UpdateManifest?> LoadManifestAsync()
 {
  ...
  {
   try{var configured=(await File.ReadAllTextAsync(FeedConfig)).Trim();if(!string.IsNullOrWhiteSpace(configured))feed=configured;}catch{}
  }
  try
  {
   using var http=new HttpClient{Timeout=TimeSpan.FromSeconds(8)};
   http.DefaultRequestHeaders.UserAgent.ParseAdd("LEAL-INFO-PDV-Updater/10.164");
   using var response=await http.GetAsync(feed);
   if(response.IsSuccessStatusCode)
   {
    var json=await response.Content.ReadAsStringAsync();
    var remoto=JsonSerializer.Deserialize<UpdateManifest>(json,JsonOptions());
    if(remoto is not null&&!string.IsNullOrWhiteSpace(remoto.Version))
    {
 ...
 private static async Task DownloadAndApplyAsync(UpdateManifest manifest,IWin32Window? owner)
 {
  ...
  using var progresso=CriarProgresso(manifest.Version,out var status,out var barra,out var percentual);progresso.Show();progresso.Refresh();
  void Atualizar(int p,string texto){p=Math.Clamp(p,0,100);barra.Value=p;percentual.Text=$"{p}%";status.Text=texto;progresso.Refresh();Application.DoEvents();}
  ...
  {
   using var http=new HttpClient{Timeout=TimeSpan.FromMinutes(4)};http.DefaultRequestHeaders.UserAgent.ParseAdd("LEAL-INFO-PDV-Updater/10.164");using var response=await http.GetAsync(uri,HttpCompletionOption.ResponseHeadersRead);response.EnsureSuccessStatusCode();var total=response.Content.Headers.ContentLength;await using var origem=await response.Content.ReadAsStreamAsync();await using var destino=new FileStream(zip,FileMode.Create,FileAccess.Write,FileShare.None,81920,true);var buffer=new byte[81920];long recebido=0;int lido;while((lido=await origem.ReadAsync(buffer))>0){await destino.WriteAsync(buffer.AsMemory(0,lido));recebido+=lido;if(total.HasValue&&total.Value>0){var p=(int)Math.Min(90,recebido*90/total.Value);Atualizar(p,"BAIXANDO ATUALIZAÇÃO...");}}Atualizar(90,"DOWNLOAD CONCLUÍDO. VERIFICANDO...");
  }
  else{var source=Path.IsPathRooted(manifest.PackageUrl)?manifest.PackageUrl:Path.Combine(UpdatesFolder,manifest.PackageUrl);if(!File.Exists(source))throw new FileNotFoundException("Pacote de atualização não encontrado.",source);File.Copy(source,zip,true);Atualizar(90,"PACOTE LOCAL CARREGADO. VERIFICANDO...");}
  ...
  Atualizar(94,"PREPARANDO ARQUIVOS...");ZipFile.ExtractToDirectory(zip,stage,true);var exe=Environment.ProcessPath??Path.Combine(AppContext.BaseDirectory,"LealInfoPDV.exe");var appDir=AppContext.BaseDirectory.TrimEnd(Path.DirectorySeparatorChar);var backupDir=Path.Combine(Database.AppFolder,"UpdateBackups","V"+CurrentVersion+"_"+DateTime.Now.ToString("yyyyMMdd_HHmmss"));Directory.CreateDirectory(backupDir);var ps=Path.Combine(work,"apply-update.ps1");var log=Path.Combine(Database.AppFolder,"Updates","apply-update.log");
  var script=$$"""
...
try {
 ...
 if (Get-Process -Id $pidToWait -ErrorAction SilentlyContinue) { Stop-Process -Id $pidToWait -Force -ErrorAction SilentlyContinue; Start-Sleep -Milliseconds 700 }
 Get-ChildItem -Path $stage -File -Recurse | ForEach-Object {
  ...
  $ok=$false
  for($i=0;$i -lt 20 -and -not $ok;$i++){ try { Copy-Item -Force $_.FullName $dest; $ok=$true } catch { Start-Sleep -Milliseconds 500 } }
  if(-not $ok){ throw "Falha ao atualizar $rel" }
 }
...
""";
  await File.WriteAllTextAsync(ps,script);Atualizar(98,"AGUARDANDO AUTORIZAÇÃO DO WINDOWS...");
  try{Process.Start(new ProcessStartInfo{FileName="powershell.exe",Arguments=$"-NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -File \"{ps}\"",UseShellExecute=true,Verb="runas",WorkingDirectory=work});}catch(System.ComponentModel.Win32Exception ex)when(ex.NativeErrorCode==1223){throw new InvalidOperationException("A atualização precisa da autorização do Windows. Clique em SIM na janela de Controle de Conta de Usuário para concluir.");}
  Atualizar(100,"INSTALANDO E REINICIANDO...");progresso.Close();
  foreach(Form form in Application.OpenForms.Cast<Form>().ToArray()){try{form.Close();}catch{}}
  Application.Exit();
  Environment.Exit(0);
 }
