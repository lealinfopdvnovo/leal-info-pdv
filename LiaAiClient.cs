...
public sealed class LiaAiClient
{
    ...
    public bool Configurada => !string.IsNullOrWhiteSpace(Environment.GetEnvironmentVariable("OPENAI_API_KEY", EnvironmentVariableTarget.User))
                              || !string.IsNullOrWhiteSpace(Environment.GetEnvironmentVariable("OPENAI_API_KEY"));

    public async Task<string?> ConversarAsync(string texto, CancellationToken cancellationToken = default)
    {
        var chave = Environment.GetEnvironmentVariable("OPENAI_API_KEY", EnvironmentVariableTarget.User)
                    ?? Environment.GetEnvironmentVariable("OPENAI_API_KEY");
        if (string.IsNullOrWhiteSpace(chave)) return null;

        var entrada = new List<object>
        {
            new { role = "system", content = PromptSistema() }
        };
        foreach (var h in historico)
            entrada.Add(new { role = h.role, content = h.text });
    ...
    private static string ExtrairTexto(JsonElement root)
    {
        ...
        foreach (var item in output.EnumerateArray())
        {
            ...
            {
                if (c.TryGetProperty("type", out var tipo) && tipo.GetString() == "output_text" &&
...
Pode conversar normalmente sobre assuntos gerais. Responda em geral com 1 a 3 frases, pois sua resposta será falada em voz alta.
""";
}
