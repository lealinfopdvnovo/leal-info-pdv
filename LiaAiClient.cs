...
public sealed class LiaAiClient
{
    ...
    public async Task<string?> ConversarAsync(string texto, CancellationToken cancellationToken = default)
    {
        ...
        if (historico.Count > 0) { entrada.AppendLine("Contexto recente, em ordem:"); foreach (var h in historico) entrada.AppendLine($"{(h.role == "user" ? "Pessoa" : "LIA")}: {h.text}"); }
        entrada.AppendLine($"Pessoa: {texto}"); entrada.Append("LIA:");
        var payload = new Dictionary<string, object?> { ["model"]="gpt-5.6-luna", ["input"]=entrada.ToString(), ["reasoning"]=new { effort="none" }, ["max_output_tokens"]=120, ["store"]=false, ["stream"]=true, ["prompt_cache_key"]="lia-pdv-voz-v154" };
        ...
        try
        {
            ...
            while(!reader.EndOfStream)
            {
                ...
                try
                {
                    ...
                    if(!root.TryGetProperty("type",out var tipo)||tipo.GetString()!="response.output_text.delta")continue;
                    ...
                    // Espera uma frase minimamente completa antes de cortar o streaming.
                    // Evita respostas truncadas e confusas causadas pelo primeiro ponto muito cedo.
                    if(parcial.Length>=55&&(parcial.EndsWith('.')||parcial.EndsWith('!')||parcial.EndsWith('?')))break;
                    if(parcial.Length>=180)break;
                }
                catch(JsonException){}
            }
            var final=resposta.ToString().Trim();if(string.IsNullOrWhiteSpace(final))return null;
            historico.Add(("user",texto));historico.Add(("assistant",final));while(historico.Count>MaxHistorico)historico.RemoveAt(0);return final;
        }
        finally
        {
            sw.Stop();
...
Converse em português do Brasil como voz ao vivo: rápida, inteligente, espontânea, descontraída e natural. A mensagem mais recente da pessoa tem prioridade. Use o contexto recente apenas para manter continuidade quando o assunto realmente for o mesmo; se a pessoa mudar de assunto, acompanhe a mudança imediatamente. Entenda intenção, contexto, gíria, referência curta e frase incompleta. Não arraste conversa comum para o PDV e não transforme conversa livre em comando. Se algo estiver realmente ambíguo, peça uma clarificação curta em vez de inventar. Responda em uma frase completa e útil; use duas somente quando necessário. Não exponha raciocínio interno.
