    ...
    private void ShowPostLoginWelcome()
    {
        try
        {
            ...
            bubble.Paint += (_, e) =>
            {
                ...
                int a = Math.Clamp((int)(216 * visualAlpha), 0, 255);
                if (a <= 0) return;
                ...
                using (var glass = new System.Drawing.Drawing2D.LinearGradientBrush(
                    rect,
                    Color.FromArgb(Math.Clamp((int)(242 * visualAlpha), 0, 255), 3, 13, 28),
                    Color.FromArgb(Math.Clamp((int)(238 * visualAlpha), 0, 255), 5, 39, 64),
                    18f))
                {
                ...
                using (var highlight = new System.Drawing.Drawing2D.LinearGradientBrush(
                    highlightRect,
                    Color.FromArgb(Math.Clamp((int)(16 * visualAlpha), 0, 255), 255, 255, 255),
                ...
                using (var glowWide = new Pen(Color.FromArgb(Math.Clamp((int)(16 * visualAlpha), 0, 255), 70, 225, 255), 10f))
                    g.DrawPath(glowWide, path);
                using (var glow = new Pen(Color.FromArgb(Math.Clamp((int)(105 * visualAlpha), 0, 255), 70, 225, 255), 3.2f))
                    g.DrawPath(glow, path);
                using (var chrome = new Pen(Color.FromArgb(Math.Clamp((int)(242 * visualAlpha), 0, 255), 205, 248, 255), 1.1f))
                ...
                {
                    int orbAlpha = Math.Clamp((int)((48 - r / 3) * visualAlpha), 0, 255);
                    using var orbBrush = new SolidBrush(Color.FromArgb(orbAlpha, 55, 218, 255));
                    g.FillEllipse(orbBrush, orbX - r, orbY - r, r * 2, r * 2);
                }
                using (var orbCore = new SolidBrush(Color.FromArgb(Math.Clamp((int)(205 * visualAlpha), 0, 255), 220, 252, 255)))
                    g.FillEllipse(orbCore, orbX - 10, orbY - 10, 20, 20);
                using (var orbRing = new Pen(Color.FromArgb(Math.Clamp((int)(90 * visualAlpha), 0, 255), 85, 235, 255), 2f))
                {
                    g.DrawEllipse(orbRing, orbX - 48, orbY - 48, 96, 96);
                    g.DrawArc(orbRing, orbX - 56, orbY - 56, 112, 112, -55, 225);
                }

                using (var techPen = new Pen(Color.FromArgb(Math.Clamp((int)(55 * visualAlpha), 0, 255), 85, 235, 255), 1.2f))
                {
                ...
                for (int i = 0; i < 4; i++)
                {
                    using var dot = new SolidBrush(Color.FromArgb(Math.Clamp((int)(90 * visualAlpha), 0, 255), 120, 240, 255));
                    g.FillEllipse(dot, rect.Right - 70 - i * 15, rect.Top + 28, 5, 5);
                }

                using var badgeBrush = new SolidBrush(Color.FromArgb(a, 180, 230, 245));
                using var titleBrush = new SolidBrush(Color.FromArgb(a, 255, 255, 255));
                using var messageBrush = new SolidBrush(Color.FromArgb(a, 255, 255, 255));
                using var brandBrush = new SolidBrush(Color.FromArgb(a, 238, 244, 250));
                using var dividerPen = new Pen(Color.FromArgb(Math.Clamp((int)(55 * visualAlpha), 0, 255), 130, 205, 225), 1f);
...
Write-Host 'Boas-vindas V10.304 aplicada: vidro escuro, contraste alto, glow discreto e fade suave.'
