...
$new=@'
            ...
            void ClipGlass(){if(panel.Width<10||panel.Height<10)return;using var cp=RoundedPath(new Rectangle(0,0,panel.Width-1,panel.Height-1),30);panel.Region?.Dispose();panel.Region=new Region(cp);}
            panel.Resize+=(_,_)=>ClipGlass();
            panel.Paint+=(_,e)=>
            {
                e.Graphics.SmoothingMode=System.Drawing.Drawing2D.SmoothingMode.AntiAlias;
                var r=new Rectangle(1,1,panel.Width-3,panel.Height-3); if(r.Width<8||r.Height<8)return;
                using var gp=RoundedPath(r,30);
                using var glass=new System.Drawing.Drawing2D.LinearGradientBrush(r,Color.FromArgb(35,105,165),Color.FromArgb(4,25,58),90f);
                ...
                using var glossBrush=new System.Drawing.Drawing2D.LinearGradientBrush(gloss,Color.FromArgb(95,220,250,255),Color.FromArgb(8,220,250,255),90f);
                e.Graphics.FillPath(glossBrush,glossPath);
                using var glow=new Pen(Color.FromArgb(0,225,255),2f); e.Graphics.DrawPath(glow,gp);
                var ir=Rectangle.Inflate(r,-4,-4); using var ip=RoundedPath(ir,26); using var hair=new Pen(Color.FromArgb(185,245,255),.8f); e.Graphics.DrawPath(hair,ip);
            };
            return panel;
'@
...
$new=@'
        ...
        {
            var b=new Button{Text=text,Dock=DockStyle.Fill,FlatStyle=FlatStyle.Flat,BackColor=Color.FromArgb(4,34,68),ForeColor=Color.White,Font=new Font("Segoe UI",10.5f,FontStyle.Bold),Cursor=Cursors.Hand,Margin=new Padding(4),TextAlign=ContentAlignment.MiddleCenter};
            b.FlatAppearance.BorderSize=0; b.FlatAppearance.MouseOverBackColor=Color.FromArgb(10,78,128); b.FlatAppearance.MouseDownBackColor=Color.FromArgb(8,98,155);
            b.Resize+=(_,_)=>{if(b.Width<10||b.Height<10)return;using var bp=RoundedPath(new Rectangle(0,0,b.Width-1,b.Height-1),24);b.Region?.Dispose();b.Region=new Region(bp);};
            b.Paint+=(_,e)=>{e.Graphics.SmoothingMode=System.Drawing.Drawing2D.SmoothingMode.AntiAlias;var br=new Rectangle(1,1,b.Width-3,b.Height-3);if(br.Width<8||br.Height<8)return;using var bp=RoundedPath(br,24);using var bg=new System.Drawing.Drawing2D.LinearGradientBrush(br,Color.FromArgb(18,92,150),Color.FromArgb(3,30,66),90f);e.Graphics.FillPath(bg,bp);using var edge=new Pen(Color.FromArgb(0,220,255),1.7f);e.Graphics.DrawPath(edge,bp);TextRenderer.DrawText(e.Graphics,b.Text,b.Font,br,Color.White,TextFormatFlags.HorizontalCenter|TextFormatFlags.VerticalCenter|TextFormatFlags.WordBreak);};
            return b;
        }
        var banco=ToolButton("ALTERAR\r\nBANCO DE DADOS");
        var corrigir=ToolButton("CORRIGIR\r\nSISTEMA");
...
# O host de input pode variar entre builds; converte o Paint por regex sem abortar a publicacao.
$pattern='(?s)\s*var rect=new Rectangle\(1,1,host\.Width-3,host\.Height-3\);.*?e\.Graphics\.DrawPath\(pen,gp\);'
$replacement='`r`n                int y=host.Height-3;`r`n                using var glow=new Pen(focused ? Color.FromArgb(0,235,255) : Color.FromArgb(70,165,210), focused ? 2.5f : 1.2f);`r`n                e.Graphics.DrawLine(glow,5,y,host.Width-6,y);'
...
Write-Host 'V10.261: renderizacao premium aplicada somente ao LoginForm.'